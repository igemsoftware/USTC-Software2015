...
class reaction_system(object):
    ...
    def set_species_name(self,speciesname):
        ...
        Returns
            none
        '''
        self.species_name=numpy.array(speciesname)
    ...
    def set_reactions(self,reaction):
        ...
        self.reactant_data=map(lambda temp1:itemfreq(temp1),(map(lambda temp1:map(lambda temp2:self.species_name_inverse[temp2],temp1),self.reactant)))
        self.product_data=map(lambda temp1:itemfreq(temp1),(map(lambda temp1:map(lambda temp2:self.species_name_inverse[temp2],temp1),self.product)))
    ...
    def simulate(self,initial,stop_time):
        ...
        def intensity(single_reactant,current):
            return numpy.array(map(lambda species_temp:scipy.misc.comb(current[species_temp[0]],species_temp[1]),single_reactant)).prod()
        def intensity_list(reactant,current):
            return map(lambda single_reactant:intensity(single_reactant,current),reactant)
        initial+=[0]*(self.species_number-len(initial))
