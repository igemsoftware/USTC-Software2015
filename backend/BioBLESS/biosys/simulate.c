  ...
  )
{
  ...
  while(t<stop_time){
    ...
    t-=log(rand()/RAND_MAX)/possibility_sum;
    ...
    num++;
    ans = (struct crt_data *)realloc(ans,(num+1)*crt_size);
    (ans+num)->time=t;
    ...
    for(int i=0;i<(rct_link+next_reaction)->reac_num;i++){
      ...
      *(possibility+*((rct_link+next_reaction)->reacs+i))=*(constant+*((rct_link+next_reaction)->reacs+i));
      for(int j=0;j<(reactant_data+*((rct_link+next_reaction)->reacs+i))->sub_num;j++){
          *(possibility+*((rct_link+next_reaction)->reacs+i))*=*(current+*((reactant_data+*((rct_link+next_reaction)->reacs+i))->subs+j));
        };
      possibility_sum+=*(possibility+*((rct_link+next_reaction)->reacs+i));
    };
  };
