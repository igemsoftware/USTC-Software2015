...
this.draw = function(devices, w, h){
		var that = this;
        ...
        onDragEnd = function() {
			...
			if(!(this.position.x + 75 >= w - 220 && this.position.x + 75 <= w - 20 && this.position.y >= 110 && this.position.y <= h - 20)){
				...
				that.stage.child_stage.addChild(elements[elements.length - 1]);
			};
        ...
        onDragMove = function() {
            if(this.dragging) {
                ...
                this.position.x = newPosition.x - 75;
                this.position.y = newPosition.y - 35;
				this.childPosition = this.data.getLocalPosition(that.stage.child_stage);
            }
        };
		...
		/*onDragStart_d = function(event) {
			...
			that.stage.child_stage.startX = that.stage.child_stage.position.x;
			that.stage.child_stage.startY = that.stage.child_stage.position.y;
        };
        ...
        onDragMove_d = function(event) {
            if(this.dragging) {
                var newPosition = this.data.getLocalPosition(this.parent);
                that.stage.child_stage.position.x = newPosition.x - this.startX + that.stage.child_stage.startX;
                that.stage.child_stage.position.y = newPosition.y - this.startY + that.stage.child_stage.startY;
            };
			that.stage.child_stage.inPosition = event.data.getLocalPosition(that.stage.child_stage);
        };*/
		...
		that.backButton.on('mousedown', function(){
			...
			that.stage.removeChild(that.stage.child_stage);
			that.stage.child_stage = that.backupStage;
			that.stage.addChild(that.stage.child_stage);
